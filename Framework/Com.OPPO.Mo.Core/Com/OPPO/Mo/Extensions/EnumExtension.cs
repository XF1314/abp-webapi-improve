using Com.OPPO.Mo.DataAnotation;
using System;
    ...
    public static class EnumExtension
    {
        ...
        public static List<EnumDescriptionDto> GetDescriptionToList<T>() where T : struct
        {
            ...
            foreach (int item in System.Enum.GetValues(type))
            {
                ...
                list.Add(new EnumDescriptionDto { Text = description, Type = item });
            }
            return list;
        }
        ...
        /// <returns></returns>
        public static List<EnumSelectListItem> EnumToSelectList(Type enumType, string firstText = "请选择", string firstValue = "-1")
        {
            List<EnumSelectListItem> listItem = new List<EnumSelectListItem>();

            if (enumType.IsEnum)
            {
                AddListItemFirst(listItem, firstText, firstValue);
                ...
                if (null != values && values.Length > 0)
                {
                    ...
                    foreach (var item in values)
                    {
                        ...
                        else
                        {
                            ...
                            desc = attribute != null ? attribute.Description : item.ToString();
                        }
                        listItem.Add(new EnumSelectListItem { Value = ((int)item).ToString(), Text = desc });
                    }
                }
            }
            else
        ...
        private static void AddListItemFirst(List<EnumSelectListItem> listItem, string firstText, string firstValue)
        {
            if (!string.IsNullOrWhiteSpace(firstText))
            {
                if (string.IsNullOrWhiteSpace(firstValue))
                    firstValue = "-1";
                listItem.Add(new EnumSelectListItem { Text = firstText, Value = firstValue });
            }
        }
    }
