...
{
    public class ActionSoftWebApiParameterSignFilter : ApiActionFilterAttribute
    {
        private readonly Lazy<IConfiguration> _configuration;
        private readonly Lazy<ILoggerFactory> _loggerFactory;

        public ActionSoftWebApiParameterSignFilter(Lazy<ILoggerFactory> loggerFactory, Lazy<IConfiguration> configuration)
        {
        ...
        public override async Task OnBeginRequestAsync(ApiActionContext apiActionContext)
        {
            try
            {
                ...
                {
                    if (apiActionContext.ApiActionDescriptor.Parameters[0].Value is ActionSoftWebApiAccessKeyInfo)
                    {
                        var signatureBasedEsbRequest = apiActionContext.ApiActionDescriptor.Parameters[0].Value as ActionSoftWebApiAccessKeyInfo;
                        if (string.IsNullOrWhiteSpace(signatureBasedEsbRequest.AccessKey))
                            signatureBasedEsbRequest.AccessKey = _configuration.Value[BpmSettingNames.ActionSoftWebApiAccessKey];
        ...
        private async Task FullfillSignatureInfo(ApiActionContext apiActionContext, string apiPath)
        {
            ...
            var actionSoftSignKey = _configuration.Value[BpmSettingNames.ActionSoftWebApiSignKey];
            ...
            if (messageContent != null)
            {
                ...
                else if (contentType == WebApiClientConsts.FormUrlEncodeContentMediaType)
                {
                    ...
                    var sign = GetHmacSignature(@params, actionSoftSignKey);
                    @params.Add(MoBpmConsts.ActionSoftSignParameterName, sign);
                ...
                else if (contentType.Contains(WebApiClientConsts.JsonContentMediaType))//"application/json; charset=utf-8"
                {
                    ...
                    var sign = GetHmacSignature(@params, actionSoftSignKey);
                    jObject.Add(MoBpmConsts.ActionSoftSignParameterName, sign);
                    apiActionContext.RequestMessage.Content.Dispose();
            ...
            else
            {
                ...
                uriEditor.AddQuery(MoBpmConsts.ActionSoftSignParameterName, sign);
                apiActionContext.RequestMessage.RequestUri = uriEditor.Uri;
            }
        }

        private static string GetHmacSignature(IDictionary<string, string> @params, string secret)
        {
            var stringBuilder = new StringBuilder();
            stringBuilder.Append(secret);
            foreach (var param in @params.OrderBy(t => t.Key, new StringComparer()))
            {
                stringBuilder.AppendFormat("{0}{1}", param.Key, param.Value);
            }

            return stringBuilder.ToString().HmacMd5Digest(secret);
        }
