...
{
    public class EsbLmsParamSignatureFilter : ApiActionFilterAttribute
    {
        private readonly Lazy<IConfiguration> _configuration;
        private readonly Lazy<ILoggerFactory> _loggerFactory;

        public EsbLmsParamSignatureFilter(Lazy<ILoggerFactory> loggerFactory, Lazy<IConfiguration> configuration)
        {
            ...
            try
            {
                    ...
                    && apiActionContext.ApiActionDescriptor.Parameters[0].Value is ISignatureBasedRequest)
                {
                    if (apiActionContext.ApiActionDescriptor.Parameters[0].Value is IAppIdInfo)
                    {
                        ...
                        {
                            appIdInfo.AppId = _configuration.Value[ThirdpartySettingNames.LmsAppId];
                        }
        ...
        private async Task FullfillSignatureInfo(ApiActionContext apiActionContext, string apiPath)
        {
            ...
            var esbSignKey = _configuration.Value[ThirdpartySettingNames.LmsWebApiAppSecret];
