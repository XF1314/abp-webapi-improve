...
{
    public class RewarSignatureFilter : ApiActionFilterAttribute
    {
        private readonly Lazy<IConfiguration> _configuration;
        private readonly Lazy<ILoggerFactory> _loggerFactory;

        public RewarSignatureFilter(Lazy<ILoggerFactory> loggerFactory, Lazy<IConfiguration> configuration)
        {
        ...
        private async Task FullfillSignatureInfo(ApiActionContext apiActionContext, string apiPath)
        {
            ...
            if (messageContent != null)
            {
                ...
                else if (contentType.Contains(WebApiClientConsts.JsonContentMediaType))//"application/json; charset=utf-8"
                {
                    ...
                    var jsonString = JsonConvert.SerializeObject(apiActionContext.ApiActionDescriptor.Parameters[0].Value);
                    ...
                    apiActionContext.RequestMessage.Headers.Add("Accept-Encoding", "gzip");

                    apiActionContext.RequestMessage.Headers.Add("Accept-Encoding", "deflate");

                    apiActionContext.RequestMessage.Headers.Add("Accept-Encoding", "br");
