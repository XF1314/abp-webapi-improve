...
namespace Com.OPPO.Mo.Thirdparty.DataGrand.Requests
{
    ...
    /// 达观搜索便签数据查询请求模型
    /// </summary>
    public class DataGrandMailDataQueryRequest : BaseDataGrandDataRequest
    {
        /// <summary>
        /// <see cref="DataGrandModuleDataQueryRequest"/>
        /// </summary>
        /// <param name="languageType"><see cref="MO2DataGrandLanguageType"/></param>
        /// <param name="mailSource"><see cref="DataGrandMailSource"/></param>
        public DataGrandMailDataQueryRequest(MO2DataGrandLanguageType languageType = MO2DataGrandLanguageType.Default, DataGrandMailSource mailSource = DataGrandMailSource.all)
        {
        ...
        /// </summary>
        [JsonIgnore]
        public DataGrandMailSource MailSource { get; private set; }
        ...
        public string Filter
        {
            get
            {
                if (MailSource == DataGrandMailSource.all && ExactFilterItems.ContainsKey(DataGrandMailDataAddInputItem.MAIL_TYPE))
                    ExactFilterItems.Remove(DataGrandMailDataAddInputItem.MAIL_TYPE);
                else if (MailSource != DataGrandMailSource.all)
                {
                    if (ExactFilterItems.ContainsKey(DataGrandMailDataAddInputItem.MAIL_TYPE))
                        ExactFilterItems[DataGrandMailDataAddInputItem.MAIL_TYPE] = MailSource.ToString();
                    else
                    {
                        ExactFilterItems.Add(DataGrandMailDataAddInputItem.MAIL_TYPE, MailSource.ToString());
                    }
                }

                var languageFilter = string.Format("{0},{1}", (int)LanguageType, (int)MO2DataGrandLanguageType.Default);
                if (ExactFilterItems.ContainsKey(DataGrandMailDataAddInputItem.LANGUAGE_TYPE))
                    ExactFilterItems[DataGrandMailDataAddInputItem.LANGUAGE_TYPE] = languageFilter;
                else
                {
                    ExactFilterItems.Add(DataGrandMailDataAddInputItem.LANGUAGE_TYPE, languageFilter);
                }
