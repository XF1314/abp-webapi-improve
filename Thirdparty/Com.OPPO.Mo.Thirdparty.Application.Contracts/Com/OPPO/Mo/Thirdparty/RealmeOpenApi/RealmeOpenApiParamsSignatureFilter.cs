...
{
    public class RealmeOpenApiParamsSignatureFilter : ApiActionFilterAttribute
    {
        private readonly Lazy<IConfiguration> _configuration;
        private readonly Lazy<ILoggerFactory> _loggerFactory;

        public RealmeOpenApiParamsSignatureFilter(Lazy< ILoggerFactory>loggerFactory, Lazy<IConfiguration> configuration)
        {
        ...
        public override async Task OnBeginRequestAsync(ApiActionContext apiActionContext)
        {
            try
            {
                ...
                var apiPath = apiActionContext.RequestMessage.RequestUri.AbsolutePath.Replace(@"/rf/",@"/");
                    ...
                    && apiActionContext.ApiActionDescriptor.Parameters[0].Value is ISignatureBasedRequest)
                {
                    if (apiActionContext.ApiActionDescriptor.Parameters[0].Value is IAppIdInfo)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(appIdInfo.AppId))
                            appIdInfo.AppId = _configuration.Value[ThirdpartySettingNames.RealmeOpenApiAppId];
        ...
        private async Task FullfillSignatureInfo(ApiActionContext apiActionContext, string apiPath)
        {
            ...
            var realmeOpenApiSignKey = _configuration.Value[ThirdpartySettingNames.RealmeOpenApiSignKey];
            ...
            if (messageContent != null)
            {
                ...
                else if (contentType == WebApiClientConsts.FormUrlEncodeContentMediaType)
                {
                    ...
                    var sign = GetSignature(@params, apiPath, realmeOpenApiSignKey);
                    @params.Add(ThirdpartyConsts.RealmeOpenApiSignParameterName, sign);
                ...
                else if (contentType.Contains(WebApiClientConsts.JsonContentMediaType))//"application/json; charset=utf-8"
                {
                    ...
                    var sign = GetSignature(@params, apiPath, realmeOpenApiSignKey);
                    jObject.Add(ThirdpartyConsts.RealmeOpenApiSignParameterName, sign);
                    apiActionContext.RequestMessage.Content.Dispose();
            ...
            else
            {
                ...
                uriEditor.AddQuery(ThirdpartyConsts.RealmeOpenApiSignParameterName, sign);
